...
| `create_news_draft`   | `title` (required), `content` (required), `titleFr?`, `contentFr?`, `imageUrl?` (URL), `slug?`, `type?` (`short` \| `article`) | Write **English** title and body. Optionally add **French** `titleFr` / `contentFr` on the same article. When mentioning a player or team, you MUST use `:player{slug="…" label="…"}` and `:team{slug="…" label="…"}` (resolve slugs with `search_players` / `search_teams`). When the primary source is an X post, put `:tweet{url="https://x.com/…/status/…"}` on its own line in **both** bodies. Creates a **draft** (`isDraft: true`). `type` defaults to **`short`** (follow the scene). Use `article` for analysis, context, or stories. Returns `adminEditUrl` for review. Slug is generated from the English title when omitted. |
| `update_news_article` | `idOrSlug` (required), `title?`, `content?`, `titleFr?`, `contentFr?`, `imageUrl?`, `slug?`, `type?`                           | Patch only the fields to change. Pass `null` for `titleFr`, `contentFr`, or `imageUrl` to clear them. Does **not** publish. Same player/team MDC tags as create.                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
3. Call create_news_draft once via SARPBC MCP:
   ...
   - type = "short" (roster changes are short news, not long-form articles)
