...
- **Discussion threads**: Wrap the thread in `SCard` (full border, not flush-bottom). No `border-b` dividers between comments. Separate top-level threads stack with `gap-row`; nested replies in the same thread sit at minimum gap. `CommentThreadConnector` aligns its branch to the author row (`h-row-compact`); last sibling is an L, earlier siblings keep a continuous spine; the reply composer is an L when it is the only child. Primary action is Reply; permalink / report / moderation live in a more-actions popover.
