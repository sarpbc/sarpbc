...
const { getPlayerById, updatePlayer } = usePlayer();
const { getPlayerContracts, createPlayerContract, updatePlayerContract, deletePlayerContract } =
  usePlayerContract();
...
const { data: playerData } = getPlayerById(playerId);
...
const contractColumns: TableColumn<PlayerContract>[] = [
  {
    accessorKey: "team",
    header: t("page.dashboard.players.contracts.columns.team"),
    cell: ({ getValue }) => (getValue() as PlayerContract["team"]).name,
  },
  {
    accessorKey: "role",
    header: t("page.dashboard.players.contracts.columns.role"),
    cell: ({ getValue }) => t(`common.contractRole.${getValue() as ContractRole}`),
  },
  {
    accessorKey: "startDate",
    header: t("page.dashboard.players.contracts.columns.startDate"),
    cell: ({ getValue }) => formatDate(getValue() as string),
  },
  {
    accessorKey: "endDate",
    header: t("page.dashboard.players.contracts.columns.endDate"),
    cell: ({ getValue }) => formatDate(getValue() as string | null),
  },
];

const { data: contractsData, refresh: refreshContracts } = getPlayerContracts(playerId);
...
const contractModalTitle = computed(() =>
  editingContract.value
    ? t("page.dashboard.players.contracts.edit.title")
    : t("page.dashboard.players.contracts.create.title"),
);
</script>
  ...
  <NuxtLayout name="dashboardheader">
    ...
    <template #action>
      <UButton
        ...
        :label="$t('page.dashboard.players.save')"
      ...
      <div class="flex flex-col gap-6">
        ...
        <div>
          <h2 class="text-base font-semibold mb-3">
            {{ $t("page.dashboard.players.info.title") }}
          </h2>
        ...
        <div>
          <div class="flex flex-row justify-between items-center mb-3">
            <h2 class="text-base font-semibold">
              {{ $t("page.dashboard.players.contracts.title") }}
            </h2>
            <UButton
              icon="i-fluent-add-24-regular"
              size="sm"
              :label="$t('page.dashboard.players.contracts.create.title')"
              class="cursor-pointer"
          ...
          <p v-if="contracts.length === 0" class="text-muted text-sm mt-2">
            {{ $t("page.dashboard.players.contracts.empty") }}
          </p>
        </div>
      </div>
    </DashboardContent>
    ...
    >
      <template #body>
        <div class="flex flex-col gap-3">
          <UFormField :label="$t('page.dashboard.players.contracts.fields.teamId')" required>
            <UInput v-model="contractForm.teamId" class="w-full" />
          </UFormField>
          <UFormField :label="$t('page.dashboard.players.contracts.fields.role')" required>
            <USelect
          ...
          <UFormField :label="$t('page.dashboard.players.contracts.fields.startDate')" required>
            <UInput v-model="contractForm.startDate" type="date" class="w-full" />
          </UFormField>
          <UFormField :label="$t('page.dashboard.players.contracts.fields.endDate')">
      ...
      <template #footer>
        <UButton
          icon="i-fluent-save-24-regular"
          :label="$t('page.dashboard.players.contracts.save')"
    ...
    <UModal
      ...
      :title="$t('page.dashboard.players.contracts.delete.title')"
      :dismissible="!isDeletingContract"
    >
      <template #body>
        <p>
          {{
            $t("page.dashboard.players.contracts.delete.confirm", {
      ...
      <template #footer>
        <UButton
          icon="i-fluent-delete-24-regular"
          color="error"
          :label="$t('page.dashboard.players.contracts.delete.confirm_button')"
