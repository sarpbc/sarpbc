...
const columns: TableColumn<Team>[] = [
  {
    accessorKey: "name",
    header: t("page.dashboard.teams.columns.name"),
  },
  {
    accessorKey: "location",
    header: t("page.dashboard.teams.columns.location"),
...
function selectRow(e: Event, row: TableRow<Team>) {
  e.preventDefault();
  navigateTo(localePath(`/dashboard/teams/${row.original.id}`));
}
...
async function confirmCreate() {
  ...
  try {
    ...
    if (created) {
      ...
      navigateTo(localePath(`/dashboard/teams/${created.id}`));
    }
  } catch (error) {
  ...
  <NuxtLayout name="dashboardheader">
    ...
    <template #action>
      <UButton
        ...
        :label="$t('page.dashboard.teams.create.title')"
      ...
      <ClientOnly>
        ...
        <div v-if="total > limit" class="w-full flex flex-row justify-between mt-4">
          <span class="text-muted">
            {{ t("page.dashboard.teams.teamsCount", { count: total }) }}
          </span>
          <UPagination
            :page="page"
            :total="total"
    ...
    <UModal
      ...
      :title="$t('page.dashboard.teams.create.title')"
    ...
    >
      ...
      <template #footer>
        <UButton
          ...
          :label="$t('page.dashboard.teams.create.save')"
          :loading="isCreating"
    ...
    <UModal
      ...
      :title="$t('page.dashboard.teams.delete.title')"
      :dismissible="!isDeleting"
    >
      <template #body>
        <p>
          {{
            $t("page.dashboard.teams.delete.confirm", {
      ...
      <template #footer>
        <UButton
          icon="i-fluent-delete-24-regular"
          color="error"
          :label="$t('page.dashboard.teams.delete.confirm_button')"
          :loading="isDeleting"
          class="cursor-pointer"
          @click="confirmDelete"
        />
